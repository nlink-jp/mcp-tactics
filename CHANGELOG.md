...
- `media.md`: the scribes no longer switch delivery mode. The result carries the
  transcript up to `max_bytes` (default 65536, `0` = no cap) and counts what the
  cap left out; the file holds all of it either way. "A short transcript comes
  back inline, a long one as a path with an excerpt" described a mechanism
  withdrawn in voice-scribe ADR-0011 / gem-scribe ADR-0003.
